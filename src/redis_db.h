...
class Database {
 public:
  explicit Database(Redis::Storage* storage, int64_t table_id, rocksdb::WriteBatch* batch);
...
class SubKeyScanner : public Redis::Database {
 public:
  explicit SubKeyScanner(Redis::Storage *storage, int64_t table_id, rocksdb::WriteBatch* batch)
      : Database(storage, table_id, batch) {}
